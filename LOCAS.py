...
def hlavni_vypocet():
    ...
    #################### FUNKCE TEPLOTA A PŘETUP TEPLA ####################

    # def fnctAirTlak(time):
    #     tlak = 0.1
    #     if (Ti0 == 42):
    #         if (time <= 1):
    #             tlak = (0.1)
    ...
    #         elif (time <= 111):
    #             rt = time/100
    #             tlak = -4.7*power(rt,6) + 17.9*power(rt,5) - 27.35*power(rt,4) + 22.1*power(rt,3) - 11.12*power(rt,2) + 4.1*rt + 0.178
    #         elif (time <= 10800):
    #             logt = log10(time)
    #             tlak = -0.06347*power(logt,4) + 0.7949*power(logt,3) - 3.606*power(logt,2) + 6.874*logt - 3.553
    #         elif (time <= 100000):
    #             tlak = (0.880713312 - 0.039760812*(time-10800)/89200)
    #         elif (time <= 200200):
    #             tlak = (0.8409525 - 0.015090125*(time-100000)/100200)
    #         else:
    #             tlak = (0.825862375 - 0.012893813*(time-200200)/452800)
    #     elif (Ti0 == 52):
    #         if (time <= 1):
    #             tlak = (0.1)
    #             return tlak
    #         elif (time <= 112):
    #             rt = time/100
    #             tlak = -4.5*power(rt,6) + 17.14*power(rt,5) - 26.4*power(rt,4) + 21.6*power(rt,3) - 11*power(rt,2) + 4.1*rt + 0.179
    #         elif (time <= 10800):
    #             logt = log10(time)
    #             tlak = -0.0619*power(logt,4) + 0.7756*power(logt,3) - 3.524*power(logt,2) + 6.74*logt - 3.477
    #         elif (time <= 100000):
    #             tlak = (0.901527562 - 0.0369015*(time-10800)/89200)
    #         elif (time <= 200200):
    #             tlak = (0.864626062 - 0.013487124*(time-100000)/100200)
    #         else:
    #             tlak = (0.851138938 - 0.010877438*(time-200200)/452800)
    #     elif (Ti0 == 72):
    #         if (time <= 1):
    #             tlak = (0.1)
    #             return tlak
    #         elif (time <= 11):
    #             rt = time/10
    #             tlak = (-1*power(rt,4) + 3.3*power(rt,3) - 4.12*power(rt,2) + 2.371*rt - 0.0981)
    #         elif (time <= 10800):
    #             logt = log10(time)/10
    #             tlak = (8618*power(logt,6) - 13160*power(logt,5) + 7911*power(logt,4) - 2358.3*power(logt,3) + 361.81*power(logt,2) - 27.544*logt + 1.2766)
    #         elif (time <= 100000):
    #             tlak = (0.280575 - 0.011689*(time-10800)/89200)
    #         elif (time <= 200200):
    #             tlak = (0.268886 - 0.005110*(time-100000)/100200)
    #         else:
    #             tlak = (0.263776 - 0.006318*(time-200200)/452800)
    ...
    def fluxFire(Tair, Tnode):
        Tair = kelvin(Tair)
        Tnode = kelvin(Tnode)
        # hc = clcConvection(Lair, Tnode, Tair)
        # hr = esbFcoef*esb*(power(Tnode,4)-power(Tair,4))/(Tnode-Tair)
        # print('hci = ' + str(hc))
        # print('hri = ' + str(hr))
        # print('Ri = ' + str(1/(hc+hr)))
    ...
    def fluxCool(Tair, Tnode):
        ...
        Tnode = kelvin(Tnode)
        # hc = clcConvection(Lair, Tnode, Tair)
        # hr = esbCcoef*esb*(power(Tnode,4)-power(Tair,4))/(Tnode-Tair)
        # print('hce = ' + str(hc))
        # print('hre = ' + str(hr))
        # print('Re = ' + str(1/(hc+hr)))
    ...
    #################### FUNKCE STACIONÁRNÍ VEDENÍ TEPLA ####################
    def conducAir(Ta):
        Ta = Ta - 273.15        # °C!
    ...
    # def clcRprestup(celsius,wind):
    #     # Výpočet součinitele přestupu tepla dle ČSN EN ISO 6946 (Příloha C) pro vnější povrch:
    #     T = celsius+273.15
    #     hr = emiss*4*stefBolt*power(T,3)    # Radiace
    #     hc = 4+4*wind                       # Proudění
    #     h = hr+hc
    #     R = 1/h
    #     return R
    ...
    lblVersion = 'final2'      # Pojmenování výstupních souborů
    Ti0 = 44               # Teplota v interiéru před LOCA; 41 or 44
    ...
    predpeti = 0            # Uvažovat předpětí? 1 (ano) or 0 (ne) ... pureTemp = 0; teplotaTlakPredpeti = 1
    pretlak = 0             # Uvažovat přetlak? 1 (ano) or 0 (ne) ... pureTemp = 0; teplotaTlakPredpeti = 1
    ...
    if stadium == 0:
        ...
        predpinaciNapeti = sigmaPinf*predpeti
        prssSftCoeff = 1.5*pretlak
        lblType = 'tInf'
    ...
    lblVersion = lblVersion + '-' + str(Ti0) + '-' + str(stadium)
    ...
    # print('')
    # print('')
    # print(conducSteel(40))            # Odpor oceli v t=0
    # print(conduc(42))           # Odpor betonu v t=0
    # print('')
    # exit

    # Ri = clcRprestup(Ti0,vitrInProvoz)          # Odpor vzduchu v interiéru
    # Re = clcRprestup(Te,vitrOutProvoz)          # Odpor vzduchu v exteriéru
    # Tstart = statHeatTrans()
    if (provozniNapeti):
    ...
    # if (porovnaniUJV == 1):
    #     tVec1mm = zeros((steps+1))
    #     tVec10mm = zeros((steps+1))
    #     tVec20mm = zeros((steps+1))
    #     tVec50mm = zeros((steps+1))
    #     tVec100mm = zeros((steps+1))
    #     tVec200mm = zeros((steps+1))
    #     tVec500mm = zeros((steps+1))
    #     tVec999mm = zeros((steps+1))
    #     tVecGrad = zeros((steps+1))
    #     tVec1mm[0] = Tstart[1]
    #     tVec10mm[0] = Tstart[10]
    #     tVec20mm[0] = Tstart[20]
    #     tVec50mm[0] = Tstart[50]
    #     tVec100mm[0] = Tstart[100]
    #     tVec200mm[0] = Tstart[200]
    #     tVec500mm[0] = Tstart[500]
    #     tVec999mm[0] = Tstart[1499]
    #     tVecGrad[0] = Tstart[1]-Tstart[1499]
    ...
    for i in range(steps):
        # 0) Začínáme na i=0.
        # 1) V daném časovém kroce (nyní) stanovíme teploty Tn a matice K a C.
        ...
        # if (porovnaniUJV == 1):
        #     tVec1mm[i+1] = Tnts[1]
        #     tVec10mm[i+1] = Tnts[10]
        #     tVec20mm[i+1] = Tnts[20]
        #     tVec50mm[i+1] = Tnts[50]
        #     tVec100mm[i+1] = Tnts[100]
        #     tVec200mm[i+1] = Tnts[200]
        #     tVec500mm[i+1] = Tnts[500]
    ...
    for i in range(steps+1):
        ...
        # >>>>>>>>>>>>>>>>>> CO TY NAPĚTÍ POČÍTAT RADŠI PRO ELEMENTY A NE UZLY? <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        for j in nodesRange:
        ...
        for j in nodesRange:
            ...
            if ((steelThick != 0) and (j < indSl)):
                # VYLEPSIT: bere to, ze na STYKU je ocel, ale pritom je tam styk ocel/beton - kurvi to pak trochu vypocet N a M
                # mozna tak ze by se to bralo po stredech elementu a ne po ulzech?
        ...
        for j in nodesRange:
            ...
            if ((steelThick != 0) and (j < indSl)):
                # OPRAVIT: bere to, ze na '=' je ocel, ale pritom je zde styk ocel/beton - kurvi to pak graf
    ...
    # if (porovnaniUJV == 1):
    #     if ((Ti0 == 41) or (Ti0 == 44)):
    #         ujvSavePath = 'figUJV/'+lblTemp+'C_'+lblTime+'s_v'+lblVersion
    #         Path(ujvSavePath).mkdir(parents=True, exist_ok=True)
    #         # Original UJV DATA
    #         if (Ti0 == 41):
    #             data = read_excel(r'UJVTC41C.xlsx')
    #         elif (Ti0 == 44):
    #             data = read_excel(r'UJVTC44C.xlsx')
    #         columns = array(DataFrame(data, columns= ['x1','y1','x10','y10','x20','y20','x50','y50','x100','y100','x200','y200','x500','y500','x999','y999']))
    #         sizes = shape(columns)
    #         x1 = array([columns[i][0] for i in range(sizes[0])])
    #         y1 = array([columns[i][1] for i in range(sizes[0])])
    #         x10 = array([columns[i][2] for i in range(sizes[0])])
    #         y10 = array([columns[i][3] for i in range(sizes[0])])
    #         x20 = array([columns[i][4] for i in range(sizes[0])])
    #         y20 = array([columns[i][5] for i in range(sizes[0])])
    #         x50 = array([columns[i][6] for i in range(sizes[0])])
    #         y50 = array([columns[i][7] for i in range(sizes[0])])
    #         x100 = array([columns[i][8] for i in range(sizes[0])])
    #         y100 = array([columns[i][9] for i in range(sizes[0])])
    #         x200 = array([columns[i][10] for i in range(sizes[0])])
    #         y200 = array([columns[i][11] for i in range(sizes[0])])
    #         x500 = array([columns[i][12] for i in range(sizes[0])])
    #         y500 = array([columns[i][13] for i in range(sizes[0])])
    #         x999 = array([columns[i][14] for i in range(sizes[0])])
    #         y999 = array([columns[i][15] for i in range(sizes[0])])

    #         # Plot UJV - časová evoluce teplot v různých hloubkách konstrukce a teplotní gradient
    #         plt.figure(figsize=(16,8), dpi=300)
    #         plt.axhline(y=0, color='k')
    #         plt.axvline(x=0, color='k')
    #         plt.grid()
    #         plt.plot(x1, y1, linestyle='solid', color=[68/255, 114/255, 196/255], label = '[UJV] Temperature in 1 mm <' + str(Decimal(Decimal(amin(y1)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(y1)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(xAxisTime, tVec1mm, linestyle=(0, (3, 3)), color=[68/255, 114/255, 196/255], label = 'Temperature in 1 mm <' + str(Decimal(Decimal(amin(tVec1mm)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(tVec1mm)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(x20, y20, linestyle='solid', color=[70/255, 170/255, 197/255], label = '[UJV] Temperature in 20 mm <' + str(Decimal(Decimal(amin(y20)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(y20)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(xAxisTime, tVec20mm, linestyle=(0, (3, 3, 1, 3)), color=[70/255, 170/255, 197/255], label = 'Temperature in 20 mm <' + str(Decimal(Decimal(amin(tVec20mm)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(tVec20mm)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(x100, y100, linestyle='solid', color=[165/255, 182/255, 211/255], label = '[UJV] Temperature in 100 mm <' + str(Decimal(Decimal(amin(y100)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(y100)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(xAxisTime, tVec100mm, linestyle=(0, (3, 3, 1, 3, 1, 3)), color=[165/255, 182/255, 211/255], label = 'Temperature in 100 mm <' + str(Decimal(Decimal(amin(tVec100mm)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(tVec100mm)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(x500, y500, linestyle='solid', color=[193/255, 210/255, 167/255], label = '[UJV] Temperature in 500 mm <' + str(Decimal(Decimal(amin(y200)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(y500)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(xAxisTime, tVec500mm, linestyle=(0, (1, 2)), color=[193/255, 210/255, 167/255], label = 'Temperature in 500 mm <' + str(Decimal(Decimal(amin(tVec500mm)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(tVec500mm)).quantize(Decimal('.1')))) + '> °C')
    #         plt.title('Temperature in the wall')
    #         plt.ylabel('Temperature [°C]')
    #         plt.xlabel('Time [s]')
    #         plt.xlim((-xAxisTimeMax/100, xAxisTimeMax))
    #         plt.ylim((20, 90))
    #         plt.legend(loc='upper right')
    #         plt.savefig('figUJV/'+lblTemp+'C_'+lblTime+'_v'+lblVersion+'TC-a.png')

    #         plt.figure(figsize=(16,8), dpi=300)
    #         plt.axhline(y=0, color='k')
    #         plt.axvline(x=0, color='k')
    #         plt.grid()
    #         plt.plot(x10, y10, linestyle='solid', color=[125/255, 96/255, 160/255], label = '[UJV] Temperature in 10 mm <' + str(Decimal(Decimal(amin(y10)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(y10)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(xAxisTime, tVec10mm, linestyle=(0, (3, 3)), color=[125/255, 96/255, 160/255], label = 'Temperature in 10 mm <' + str(Decimal(Decimal(amin(tVec10mm)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(tVec10mm)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(x50, y50, linestyle='solid', color=[246/255, 146/255, 64/255], label = '[UJV] Temperature in 50 mm <' + str(Decimal(Decimal(amin(y50)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(y50)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(xAxisTime, tVec50mm, linestyle=(0, (3, 3, 1, 3)), color=[246/255, 146/255, 64/255], label = 'Temperature in 50 mm <' + str(Decimal(Decimal(amin(tVec50mm)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(tVec50mm)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(x200, y200, linestyle='solid', color=[213/255, 165/255, 164/255], label = '[UJV] Temperature in 200 mm <' + str(Decimal(Decimal(amin(y200)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(y200)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(xAxisTime, tVec200mm, linestyle=(0, (3, 3, 1, 3, 1, 3)), color=[213/255, 165/255, 164/255], label = 'Temperature in 200 mm <' + str(Decimal(Decimal(amin(tVec200mm)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(tVec200mm)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(x999, y999, linestyle='solid', color=[182/255, 171/255, 197/255], label = '[UJV] Temperature in 1499 mm <' + str(Decimal(Decimal(amin(y999)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(y999)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(xAxisTime, tVec999mm, linestyle=(0, (1, 1, 1, 1)), color=[182/255, 171/255, 197/255], label = 'Temperature in 1499 mm <' + str(Decimal(Decimal(amin(tVec999mm)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(tVec999mm)).quantize(Decimal('.1')))) + '> °C')
    #         plt.title('Temperature in the wall')
    #         plt.ylabel('Temperature [°C]')
    #         plt.xlabel('Time [s]')
    #         plt.xlim((-xAxisTimeMax/100, xAxisTimeMax))
    #         plt.ylim((20, 90))
    #         plt.legend(loc='upper right')
    #         plt.savefig('figUJV/'+lblTemp+'C_'+lblTime+'_v'+lblVersion+'TC-b.png')

    #         plt.figure(figsize=(16,8), dpi=300)
    #         plt.axhline(y=0, color='k')
    #         plt.axvline(x=0, color='k')
    #         plt.grid()
    #         plt.plot(x1, (y1-y999), linestyle='solid', color=[0, 128/255, 0], label = '[UJV] Temperature gradient: <' + str(Decimal(Decimal(amin(tVecGrad)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(tVecGrad)).quantize(Decimal('.1')))) + '> °C')
    #         plt.plot(xAxisTime, tVecGrad, linestyle='dashed', color=[0, 128/255, 0], label = 'Temperature gradient: <' + str(Decimal(Decimal(amin(tVecGrad)).quantize(Decimal('.1')))) + ',' + str(Decimal(Decimal(amax(tVecGrad)).quantize(Decimal('.1')))) + '> °C')
    #         plt.title('Temperature gradient in the wall')
    #         plt.ylabel('Temperature gradient [°C]')
    #         plt.xlabel('Time [s]')
    #         plt.xlim((-xAxisTimeMax/100, xAxisTimeMax))
    #         plt.ylim((0, 40))
    #         plt.legend(loc='upper right')
    #         plt.savefig('figUJV/'+lblTemp+'C_'+lblTime+'_v'+lblVersion+'grad.png')
